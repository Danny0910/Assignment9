...
//A route for order
$app->get('/order/{name}/{drink}/{food}', function ($name, $drink, $food) use ($app) {
    return 'Hello '.$app->escape($name).', your ordered a '.$app->escape($food).' with a '.$app->escape($drink).'!';
});

//A route for order using twig
$app->get('/order_twig/{name}/{drink}/{food}', function($name,$drink,$food) use ($app){
    return $app->render('order_twig.twig',['name' => $name, 'food' => $food, 'drink' => $drink]);
});

// A route to add two numbers
$app->get('/add/{int_a}/{int_b}',function($int_a, $int_b) use ($app){
    //calculate sum
    $sum = $int_a + $int_b;
    //build url for product
    $url = $app->url('product_route',['int_a' => $int_a, 'int_b' => $int_b]);
    //build return string
    $return_string = '<html><body>';
    $return_string .= $app->escape($int_a.' + '.$int_b.' = '.$sum);
    $return_string .= '<br/>';
    $return_string .= "Click <a href=\"$url\">here</a> to multiply $int_a and $int_b!";
    $return_string .='</body></html>';
    return $return_string;
});

// A route to multiply two numbers with name product_route
